...
std::list<float> plan(double fromlon, double fromlat, double tolon, double tolat)
{
	...
	if (status == Status::Ok)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			re.push_back(loc.lon());
			re.push_back(loc.lat());
		}
...
int main()
{
	std::list<float> r = plan(116.481028, 39.989643, 116.465302, 40.004717);

	std::list<float>::iterator itor; //定义迭代器
